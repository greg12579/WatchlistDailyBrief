...
from watchbrief.data.events import get_events
...
from watchbrief.llm.explain import explanation_to_dict, get_explanation
...
def run_daily(config: Config, target_date: date) -> None:
    ...
    for result, score, rank in ranked:
        ...
        # Get events
        events = get_events(result.ticker, start_date, target_date)

        # Get explanation
        if llm_client:
            explanation = get_explanation(result, events, llm_client)
        else:
            from watchbrief.llm.explain import create_fallback_explanation
            explanation = create_fallback_explanation(result, events)
    ...
    for item in brief_items:
        r = item.result
        emoji = "🔴" if r.label == "ACTIONABLE" else "🟡"
        print(f"{emoji} {r.ticker}: {r.pct_change_1d:+.2f}% | {r.label} | Score: {item.score:.2f}")
    print("=" * 50)
